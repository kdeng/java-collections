 ...
 */
@Component
@ConditionalOnBean(Publisher)
class SubscriberOne {

  static final Logger LOG = LoggerFactory.getLogger(SubscriberOne)
  ...
  @PostConstruct
  void init() {
    LOG.info("Initialise subscriber one")
    Connection connection = connectionFactory.createConnection()
    channel = connection.createChannel(false)
    String queueName = channel.queueDeclare().queue
    channel.queueBind(queueName, Application.EXCHANGE_NAME, 'routing.key.one')
    LOG.info("Trying to binding basic consume")
    channel.basicConsume(queueName, false, 'routing.key.one', getMyConsumer(channel))
