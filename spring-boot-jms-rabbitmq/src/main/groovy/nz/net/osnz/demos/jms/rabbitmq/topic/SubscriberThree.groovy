...
class SubscriberThree {
    ...
    static final Logger LOG = LoggerFactory.getLogger(SubscriberThree)
    ...
    @PostConstruct
    void init() {
        LOG.info("Initialise subscriber three")
        Connection connection = connectionFactory.createConnection()
        channel = connection.createChannel(false)
        String queueName = channel.queueDeclare().queue
        channel.queueBind(queueName, Application.EXCHANGE_NAME, 'routing.key.#')
        LOG.info("Trying to binding basic consume")
        channel.basicConsume(queueName, false, 'routing.key.#', getMyConsumer(channel))
