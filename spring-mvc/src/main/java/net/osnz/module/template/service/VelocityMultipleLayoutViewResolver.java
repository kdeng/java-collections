package net.osnz.module.template.service;
...
public class  VelocityMultipleLayoutViewResolver extends VelocityViewResolver {
  ...
  private static final Logger log = LoggerFactory.getLogger(VelocityMultipleLayoutViewResolver.class);
  ...
  protected AbstractUrlBasedView buildView( final String viewName ) throws Exception {
    ...
    log.debug( "Building view using multiple layout resolver. View name is " + viewName );
    ...
    if ( ! this.mappings.isEmpty() ) {
      for ( Map.Entry<String, String> entry : this.mappings.entrySet() ) {
        ...
        if ( viewName.matches( mappingRegexp ) ) {
          log.debug( "  Found matching view. Setting layout url to " + entry.getValue() );
